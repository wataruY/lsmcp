...
describe("LSP MCP integration tests", () => {
  ...
  beforeAll(async () => {
    ...
    try {
      await fs.access(path.join(__dirname, "../dist/lsp-mcp.js"));
    } catch {
      console.log("Skipping LSP MCP tests: dist/lsp-mcp.js not found. Run 'pnpm build' first.");
      return;
    }
    ...
    mcpProcess = spawn("node", [path.join(__dirname, "../dist/lsp-mcp.js")], {
      env: {
        ...process.env,
    ...
    const transport = new StdioClientTransport({
      command: "node",
      args: [path.join(__dirname, "../dist/lsp-mcp.js")],
      env: {
        ...process.env,
        LSP_COMMAND: "typescript-language-server --stdio",
        PROJECT_ROOT: tmpDir,
