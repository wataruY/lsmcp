...
export function resolveLineParameter(
  ...
  if (typeof lineParam === "number") {
    ...
    if (lineParam < 1 || lineParam > lines.length) {
      throw new Error(
        `Invalid line number: ${String(lineParam)}. File has ${String(
          lines.length
        )} lines.`
      );
    }
...
export function findSymbolInLine(
  ...
  if (lineNumber < 1 || lineNumber > lines.length) {
    throw new Error(
      `Invalid line number: ${String(lineNumber)}. File has ${String(
        lines.length
      )} lines.`
    );
  }
  ...
  if (occurrences.length === 0) {
    throw new Error(
      `Symbol "${symbolName}" not found on line ${String(lineNumber)}`
    );
  }

  if (index < 0 || index >= occurrences.length) {
    throw new Error(
      `Symbol "${symbolName}" only appears ${String(
        occurrences.length
      )} time(s) on line ${String(lineNumber)}, but index ${String(
        index
      )} was requested`
    );
  }
