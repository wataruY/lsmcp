...
async function formatFileChanges(
  ...
  const contentResult = await fs.readFile(file.filePath, 'utf-8')
  ...
  if (contentResult.isErr()) {
    ...
    const fallbackLines = file.changes.map(change => 
      `    Line ${String(change.line)}: "${change.oldText}" → "${change.newText}"`
    );
  ...
  const formattedChanges = Object.entries(changesByLine).flatMap(([lineNumStr, lineChanges]) => {
...
export async function formatRenameSymbolResult(result: RenameSymbolResult, root: string): Promise<Result<string, string>> {
  ...
  const output = [
    `${message} in ${String(changedFiles.length)} file(s) with ${String(totalChanges)} change(s).`,
    "",
    "Changes:",
  ];
...
export const renameSymbolTool: ToolDef<typeof schema> = {
  ...
  handler: async (args) => {
    ...
    const formattedResult = await formatRenameSymbolResult(result.value, args.root);
